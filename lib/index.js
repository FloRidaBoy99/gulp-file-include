    ...
    function loopHandler(inst) {
      ...
      if (args) {
        ...
        if (args[3].match(/^('|")[^']|[^"]('|")$/)) {
          ...
          var jsonfile = path.join(file.base, args[3].replace(/^('|")/, '').replace(/('|")$/, ''))
          // check if json file exists
          if (fs.existsSync(jsonfile)) {
            var fileContent = fs.readFileSync(jsonfile, 'utf-8')
            if (jsonfile.endsWith('cson')) {
              fileContent = coffeescript.compile(fileContent, {bare: true, header: false})
              while (fileContent.match(";")) {
